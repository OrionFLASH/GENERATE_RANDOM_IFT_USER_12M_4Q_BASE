...
1. **Основной файл (`result_base_*.xlsx`):**
   ...
   - Форматирование: закрепленная первая строка, автофильтр, автоматическая ширина колонок, ИНН и табельные номера как текст
...
**Методы:**
- `_preload_managers_data()` - Предзагружает данные о клиентских менеджерах для всех 12 месяцев
- `_filter_clients_for_month(month)` - Фильтрует клиентов, у которых в указанном месяце есть данные по ТБ и ГОСБ
- `_generate_up_facts(clients_df, month)` - Генерирует нарастающие факты (UP) для клиентов
- `_generate_dif_facts(clients_df, month)` - Генерирует дифференциальные факты (DIF) для клиентов
- `_add_managers_data(clients_df, month)` - Добавляет данные клиентских менеджеров из USER_CNG
- `_save_sheet_to_main_file(df, month, fact_type, excel_path, file_mode)` - Сохраняет лист в основной Excel файл
- `_save_month_sheet(df, month, fact_type)` - Сохраняет лист для указанного месяца в отдельный Excel файл
- `process()` - Полный цикл генерации листов с фактами (сначала все листы в основной файл, затем отдельные файлы для выбранных месяцев)
