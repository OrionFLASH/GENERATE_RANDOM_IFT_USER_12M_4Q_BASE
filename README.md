...
3. **Параметры выбора месяцев:**
   - `selected_months` - список словарей с параметрами:
     - `months` - список месяцев (1-12)
...
- `process()` - Полный цикл генерации листов с фактами
...
- Формат файлов: `M-{месяц}_{тип}_{timestamp}.xlsx` (например, `M-1_UP_20251112_2001.xlsx`)
- Каждый файл содержит данные только для одного месяца с одним типом факта
- В каждый файл автоматически добавляются данные клиентских менеджеров из USER_CNG
