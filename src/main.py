...
LOADER_CONFIG = {
    ...
    'FACT_SHEETS': {
        ...
        'total_rows_range': {
            ...
            'max': 200_000  # Максимальное количество строк
        },
        ...
        # Рекомендации для M4 Max (14 ядер):
        # - Минимум: 14 потоков (по количеству ядер)
        # - Оптимально: 16-24 потока (для смешанной CPU/I/O нагрузки)
        # - Максимум: 32-40 потоков (если операция записи файлов занимает много времени)
        # Текущее значение 16 - оптимальный баланс для данной задачи
    }
